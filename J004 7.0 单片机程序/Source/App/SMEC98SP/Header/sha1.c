...
unsigned long _lrol_(unsigned long c, unsigned char b)
{
	...
	temp = temp_c[0];temp = temp<<8;
	temp += temp_c[1];temp = temp<<8;
	temp += temp_c[2];temp = temp<<8;
	temp += temp_c[3]; 
    return temp;  
}  
...
void SHA1Transform( unsigned long * state /*[5]*/, unsigned char * buffer /*[64]*/ )  
{
	...
	for ( t = 0; t < 80; t ++ ) 
	{
		...
		buf_temp = a_temp[0];buf_temp = buf_temp<<8;
		buf_temp += a_temp[1];buf_temp = buf_temp<<8;
		buf_temp += a_temp[2];buf_temp = buf_temp<<8;
		buf_temp += a_temp[3]; 

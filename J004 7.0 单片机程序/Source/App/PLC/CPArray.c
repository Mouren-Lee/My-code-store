...
extern	u8 ServoClear_flag;
extern u8 Hole_flag;         //打孔标志
extern u8 Hole_flag2;   
extern u8 PuleOut_flag;
...
//	//----------------喷胶完成-------------------------
//	if(pSend->SendCnt>USART1_GetBuf(DuanJiao_CNT))
//	{
...
//	}
	...
	case CP_DECELERATE:
		...
		pSend->NextT = pSend->pArray2->pArray[pSend->Index];
	if(pSend->SendCnt>=pSend->pArray2->PlusCnt)			
		{//这时pSend->SendCnt应该为pSend->pArray->PlusCnt;
			pSend->NextT = pSend->pArray2->StopT;			
			if(pSend->SeekCnt==0)
			{
				pSend->State = CP_KEEP_LEN;
				pSend->SendCnt = pSend->KeepCnt;
			...
			{
				//pSend->State = CP_SEEK_MARK;
			//	pSend->State = CP_SEEK_MARK2;
				pSend->SendCnt = 0;
				pSend->MarkAdvanceNum = 0;
	...
	case CP_SEEK_MARK2:	
	       ...
	       pSend->MarkAdvanceNum++;  
	       if(pSend->MarkAdvanceNum>USART1_GetBuf(MarkWait_CNT)) 
				 {
					pSend->MarkAdvanceNum = 0; 
					pSend->State = CP_SEEK_MARK; 
				 }
	...
	case CP_KEEP_LEN:
	 ...
	 if(PuleOut_flag==1||PuleOut_flag==2)
		{			
	   	IO_SetDO(DO_HOLE);
		  Hole_flag=2;
		}		

...
//		PlusCnt = 0; 
  ...
  if(SetArrayParam(&CP0Array2,PlusCnt,curve,StartT,StopT,MinT,TimeScale*10/USART1_GetBuf(SPEED_CP1JianSu)))
		ArrayInit(&CP0Array2);
	//	SetArrayParam(&CP0Array,PlusCnt,curve,StartT,StopT,MinT,TimeScale);
	//	ArrayInit(&CP0Array);

	while(!ArrayRun(&CP0Array));
	while(!ArrayRun(&CP0Array2));
...
u32 Get_CP0()
{
	//return CP0Array.PlusCnt;
    //	CP0Send.SendCnt;
	// return CP0Array.DingChangCnt; 
   return CP0Send.DingChangCnt;
}
void CP0_ClearDingChang()
{
//	CP0Array.DingChangCnt = 0;
	CP0Send.DingChangCnt = 0;
}
bool CP0_isLostMark()
{
...
u16 CP0_GetHavenplus()
{
	return CP0Send.Havencnt;
}
